...
- =emacsclient=: Creates a new client frame via =emacsclient= and displays the editing buffer there.

The =frame= option is available only when using Emacs on a window system.

The =emacsclient= option is intended for users who already run Emacs as a server and want Atomic Chrome to reuse the same display/backend selection behavior as =emacsclient -c=. It currently supports buffers created with =atomic-chrome-create-file-strategy= set to =buffer=; file-backed strategies are rejected with an error.
...
#+begin_quote
[!NOTE]
When using =emacsclient=, =C-x C-c= is rebound only in Atomic Chrome buffers opened with this style so that it closes the current Atomic Chrome session instead of exiting Emacs.
#+end_quote
